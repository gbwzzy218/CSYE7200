...
object Function {
  ...
  def map2[T1, T2, R](t1y: Try[T1], t2y: Try[T2])(f: (T1, T2) => R): Try[R] =  // TODO 4
    t1y match {
      case Success(t1) => t2y match {
        case Success(t2) => Success(f(t1, t2))
  ...
  def lift2[T1, T2, R](f: (T1, T2) => R): (Try[T1], Try[T2]) => Try[R] = {
    (t1y: Try[T1], t2y:Try[T2]) => t1y flatMap(t1 => t2y map(t2 => f(t1, t2)))
  }
  ...
  def lift3[T1, T2, T3, R](f: (T1, T2, T3) => R): (Try[T1], Try[T2], Try[T3]) => Try[R] = {
    (t1y: Try[T1], t2y: Try[T2], t3y: Try[T3]) => t1y flatMap(t1 => t2y flatMap( t2 => t3y map (t3 => f(t1, t2, t3))))
  }
  ...
  def lift7[T1, T2, T3, T4, T5, T6, T7, R](f: (T1, T2, T3, T4, T5, T6, T7) => R): (Try[T1], Try[T2], Try[T3], Try[T4], Try[T5], Try[T6], Try[T7]) => Try[R] = {
    (t1y: Try[T1], t2y: Try[T2], t3y: Try[T3], t4y: Try[T4], t5y: Try[T5], t6y: Try[T6], t7y: Try[T7]) => for{
      t1 <- t1y
      t2 <- t2y
      t3 <- t3y
      t4 <- t4y
